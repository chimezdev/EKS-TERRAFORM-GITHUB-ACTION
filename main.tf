...
module "eks" {
    ...
    env                  = var.env
    cluster-name          = "${local.org}-${var.cluster-name}"
    cidr-block            = var.vpc-cidr-block
    vpc-name              = "${local.org}-${var.vpc-name}"
    igw-name              = "${local.org}-${var.igw-name}"
    pub-subnet-count      = var.pub-subnet-count
    pub-cidr-block        = var.pub-cidr-block
    pub-availability-zone = var.pub-availability-zone
    pub-sub-name          = "${local.org}-${var.pub-sub-name}"
    pri-subnet-count      = var.pri-subnet-count
    pri-cidr-block        = var.pri-cidr-block
    pri-availability-zone = var.pri-availability-zone
    pri-sub-name          = "${local.org}-${var.pri-sub-name}"
    public-rt-name        = "${local.org}-${var.public-rt-name}"
    private-rt-name       = "${local.org}-${var.private-rt-name}"
    eip-name              = "${local.org}-${var.eip-name}"
    ngw-name              = "${local.org}-${var.ngw-name}"
    eks-sg                = var.eks-sg
